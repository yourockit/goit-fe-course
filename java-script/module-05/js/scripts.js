...
const getUserByEmail = (arr, email) => {
    const emailUser = arr.find(user => user.email === email);
    return emailUser;
};
...
const getUsersWithAge = (arr, min, max) => {
    const ageUser = arr.filter(user => user.age >= min && user.age <= max);
    return ageUser;
};
...
const getTotalBalance = arr => {
    const balanceUser = arr.reduce((acc, user) => acc + user.balance, 0)
    return balanceUser;
};
...
const getUsersByFriend = (arr, name) => {
    const userByFriend = arr.filter(user => user.friends.includes(name)).map(user => user.name);
    return userByFriend;
};
